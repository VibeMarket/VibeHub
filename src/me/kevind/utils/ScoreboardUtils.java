...
public class ScoreboardUtils {
    ...
    public static void setupScoreboard(Player player) {
        ...
        Date date = new Date(player.getFirstPlayed());
            ...
            Team playerr = player.getScoreboard().registerNewTeam("player");
            playerr.setSuffix(player.getName());
            playerr.addEntry(ColorUtils.color("&7Name: &f"));
        }
        public static void updateScoreboard(Player player) {
            ...
            Team rank = player.getScoreboard().getTeam("rank");
            if (rank == null) {
                return;
            }
            rank.setSuffix(ColorUtils.color(user.getCachedData().getMetaData().getPrefix()));
